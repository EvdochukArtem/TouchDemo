...
namespace	E2 
{
	...
	public:
		...
		Vector(int cx, double cy) {
			x=(float)cx; y=(float)cy;
		}
		Vector(double cx, int cy) {
			x=(float)cx; y=(float)cy;
		}
		Vector(int cx, float cy) {
			x=(float)cx; y=(float)cy;
		}
		Vector(float cx, int cy) {
			x=(float)cx; y=(float)cy;
		}
/*
		Vector(double cx, double cy)
 ...
 */
/* ВБ: На хрен эту бомбу, пока не поздно!
 		Vector(float rho, double theta)
		{
...
MATH_API 		friend Vector operator * (float r, Vector& v) { return Vector(r*v.x, r*v.y); }
	...
	public:
		...
		Vector dest;
 		...
 		Edge & operator = (Edge &e) { org = e.org; dest = e.dest; return *this; }

 		bool operator == (Edge &e) const { return (org == e.org) && (dest == e.dest); }
 		bool operator != (Edge &e) const { return (org != e.org) || (dest != e.dest); }
...
MATH_API 		friend Vector operator * (const float r, const Vector& v) { return Vector(r*v.x, r*v.y, r*v.z); }
 		...
 		Vector rotY(double ang) const
		{
 		...
 		Vector rotXYZ(Vector &axis, float ang) const
		{
			Vector ax = axis;
			ax.norm();
			double cosa = cos(ang);
		...
		// Crossings
 		Vector	crossXY(Vector &towards) const {	// where do the line from (*this) to (towards) cross the XY plane ?
			float t=z/(z-towards.z);
			return Vector(t*(towards.x-x)+x, t*(towards.y-y)+y, 0);
		}
 		Vector	crossXZ(Vector &towards) const {	// where do the line from (*this) to (towards) cross the XY plane ?
			float t=y/(y-towards.y);
			return Vector(t*(towards.x-x)+x, 0, t*(towards.z-z)+z);
		}
 		Vector	crossYZ(Vector &towards) const {	// where do the line from (*this) to (towards) cross the XY plane ?
			float t=x/(x-towards.x);
	...
	public:
 		...
 		Operator(float c)
		{
 		...
 		Operator(E3::Vector &_x, E3::Vector &_y, E3::Vector &_z) :
		x(_x), y(_y), z(_z)
		{ }
	// Copy ctr
 		Operator(Operator &A) : x(A.x), y(A.y), z(A.z) {
		}
	// Copy 
 		Operator&  operator =	(Operator& A)
		{
...
MATH_API 		friend Vector operator * (const double r, const Vector& v) { return Vector(r*v.x, r*v.y, r*v.z); }
 		...
 		Vector rotY(double ang)
		{
 		...
 		Vector rotXYZ(Vector &axis, double ang)
		{
			Vector ax = axis;
			ax.norm();
			double cosa = cos(ang);
	...
	public:
 		...
 		Operator(float c)
		{
 		...
 		Operator(E3D::Vector &_x, E3D::Vector &_y, E3D::Vector &_z) :
		x(_x), y(_y), z(_z)
		{ }
	// Copy ctr
 		Operator(Operator &A) : x(A.x), y(A.y), z(A.z) {
...
namespace E4 {
	...
	//	In-places
		...
		// Relations
 		bool	operator ==	(Vector& v)	{ return (*this-v).norma() <= 0.0f; }
 		bool	operator !=	(Vector& v)	{ return (*this-v).norma() > 0.0f; }
 		bool	operator >	(Vector& v)	{ return x>v.x	&& y>v.y && z>v.z && w>v.w; }
 		bool	operator <	(Vector& v)	{ return x<v.x	&& y<v.y && z<v.z && w<v.w; }
 		bool	operator >=	(Vector& v)	{ return x>=v.x && y>=v.y && z>=v.z && w>=v.w; }
 		bool	operator <=	(Vector& v)	{ return x<=v.x && y<=v.y && z<=v.z && w<=v.w; }
		//
...
MATH_API 		friend Vector operator * (float r, Vector &v) { return Vector(r*v.x, r*v.y, r*v.z, r*v.w); }
		...
		//...vector
 		...
 		Vector	operator -	(Vector& v)	{ return Vector( (x-v.x), (y-v.y), (z-v.z), w-v.w); }
 		Vector	Scale(Vector& v)	{ return Vector( (x*v.x), (y*v.y), (z*v.z), (w*v.w) ); }

		// Dot product
 		float	operator *	(Vector& v)	{ return x*v.x+y*v.y+z*v.z+w*v.w; }
		...
		// Crossings
 		Vector	crossXYW(Vector &towards){	// where do the line from (*this) to (towards) cross the XY plane ?
			float t=z/(z-towards.z);
			return Vector(t*(towards.x-x)+x, t*(towards.y-y)+y, 0, t*(towards.w-w)+w);
		}
 		Vector	crossXZW(Vector &towards){	// where do the line from (*this) to (towards) cross the XY plane ?
			float t=y/(y-towards.y);
			return Vector(t*(towards.x-x)+x, 0, t*(towards.z-z)+z, t*(towards.w-w)+w);
		}
 		Vector	crossYZW(Vector &towards){	// where do the line from (*this) to (towards) cross the XY plane ?
			float t=x/(x-towards.x);
			return Vector(0, t*(towards.y-y)+y, t*(towards.z-z)+z, t*(towards.w-w)+w);
		}
 		Vector	crossXYZ(Vector &towards){	// where do the line from (*this) to (towards) cross the XY plane ?
	...
	public:
 		...
 		Operator(float c) {
			int *pint=(int*)this;
			for(int i=0; i<32; i++,pint++)	// 16*8 = 32*4
				*pint=0;
			a11=a22=a33=a44=c;
		}
 		Operator(
